...
unsigned int passengers_amount_in_airport (LayoverTable a, unsigned int h) {
  ...
  for(unsigned int i=0u; i<HOURS; i++) {
    if(a[i][0].hour == h) {
      sum = sum + a[i][1].passengers_amount;
    }
  }
...
void array_from_file(LayoverTable array, const char *filepath) {
  ...
  char code;
  int i=0;
  while (i < HOURS && !feof(file))/* COMPLETAR: lectura completa de todos los datos */{
    ...
    array[i][arrival] = flight_from_file(file, code);/* completar... */;
    array[i][departure] = flight_from_file(file, code);/* completar... */;
    fscanf(file, "\n");
    i++;
  }
  fclose(file);
