...
int main(int argc, char *argv[]) {
    ...
    /* show the historical minimum temperature */
    int min = min_temp_hist(array);
    printf("\nHistorical minimum temperature : %d\n", min);

    /* --------------------------------------- */
    int _max_temp_anual[YEARS];

    anual_max_temp(array, _max_temp_anual);

    printf("Max temp anual: ");
    printf("[");
    for(int i = 0; i < YEARS; i++){
        printf(" %d", _max_temp_anual[i]);
        if(i != YEARS-1) printf(",");
    }
    printf("]\n");
    ...
    anual_rainfall_month(array, max_rain);
